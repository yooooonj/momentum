...
interface TextAnalysis {
  ...
  emotion: 'neutral' | 'enthusiastic' | 'friendly' | 'professional' | 'playful';
  keywords: string[];
  length: number;
  emotionScore: number;
}
...
// 감정 톤 분석 함수
function analyzeEmotion(text: string): { emotion: TextAnalysis['emotion'], score: number } {
  ...
  let maxScore = 0;
  let dominantEmotion: TextAnalysis['emotion'] = 'neutral';
  ...
  Object.entries(emotionKeywords).forEach(([emotionType, keywords]) => {
    ...
    if (score > maxScore) {
      maxScore = score;
      dominantEmotion = emotionType as TextAnalysis['emotion'];
    }
  });
...
function analyzeText(text: string): TextAnalysis {
  // 문장 타입 분석
  let sentenceType: TextAnalysis['sentenceType'] = 'declarative';
...
// 톤 프로파일 생성
function createToneProfile(text: string): ToneProfile {
  const analysis = analyzeText(text);
  const recommendedChanges: ToneProfile['recommendedChanges'] = [];
...
// 텍스트 변환 적용
function applyToneChanges(text: string, selectedChanges?: string[]): string {
  let result = text;
...
const initialTextNodes = initialSelection.filter(node => node.type === 'TEXT') as TextNode[];
