...
function analyzeEmotion(text) {
    ...
    const maxEmotion = Object.entries(scores).reduce((a, b) => scores[a[0]] > scores[b[0]] ? a : b)[0];
...
// 톤 프로파일 분석 함수
function analyzeToneProfile(text) {
    ...
    let friendliness = 'neutral';
    if (warmIndicators.some(indicator => text.includes(indicator)))
        friendliness = 'warm';
    else if (coldIndicators.some(indicator => text.includes(indicator)))
        friendliness = 'cold';
    let directness = 'moderate';
    if (directIndicators.some(indicator => text.includes(indicator)))
        directness = 'direct';
    else if (indirectIndicators.some(indicator => text.includes(indicator)))
        directness = 'indirect';
...
// 텍스트 변환 적용 함수
function applyToneConversion(text, selectedPatterns) {
...
figma.ui.onmessage = (msg) => {
    switch (msg.type) {
        ...
        case 'apply-selected-patterns': {
            // 선택된 패턴만 적용
            const { selectedPatterns, nodeId } = msg.data;
            ...
            if (targetNode) {
                ...
                if (originalText !== convertedText) {
                    figma.loadFontAsync(targetNode.fontName).then(() => {
